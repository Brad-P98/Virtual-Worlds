...
void Terrain::generateXRow(glm::vec3 currentChunkPos, int row)
{
	...
	for (int i = 0; i < 2 * renderDistance + 1; i++) {
		...
		if (tessActive) shader = ShaderManager::getShader("terrain_tess_basic");
...
void Terrain::setTesselation(bool state)
{
	tessActive = state;
	//Set on/off for every active chunk
	for (int i = 0; i < activeTerrainChunks.size(); i++) {
		for (int j = 0; j < activeTerrainChunks[i].size(); j++) {
			if (activeTerrainChunks[i][j] == nullptr) return;
			if (tessActive) {
				shader = ShaderManager::getShader("terrain_tess_basic");
				activeTerrainChunks[i][j]->enableTesselation(shader);
			}
			else {
				activeTerrainChunks[i][j]->disableTesselation(ShaderManager::getShader("terrain_basic"));
			}
		}
	}
}

void Terrain::setWireFrame(bool state)
{
	wireFrameState = state;
	//Set on/off for every chunk
	for (int i = 0; i < activeTerrainChunks.size(); i++) {
		for (int j = 0; j < activeTerrainChunks[i].size(); j++) {
			if (activeTerrainChunks[i][j] == nullptr) return;

			activeTerrainChunks[i][j]->setWireFrame(wireFrameState);
		}
	}
}
