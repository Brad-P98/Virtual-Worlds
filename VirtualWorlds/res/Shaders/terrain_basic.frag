...
void main() {
	...
	vec4 perlinColour = texture(texSampler3, texCoord);

	//set initial fragment colour
	vec4 tempFragColour = gravelColour;

	//mix colour based on height
	float height = vertexPos.y;
	float mixAmount = 0.0;

	//Grassy
	mixAmount = clamp(pow(height + 21, 2) / 20, 0, 1);
	tempFragColour = mix(gravelColour, grassColour, mixAmount);

	//Snowy
	mixAmount = clamp(pow(height - 30, 1.6) /30, 0, 1);
	tempFragColour = mix(tempFragColour, snowColour, mixAmount);
